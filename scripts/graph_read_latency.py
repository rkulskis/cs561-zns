...
MODES = ["rand_read", "seq_read"]

def get_iops_from_file(filepath):
    with open(filepath) as f:
        data = json.load(f)
        return ((data["jobs"][0]["read"]["clat_ns"]["mean"] + data["jobs"][0]["read"]["slat_ns"]["mean"]) / 1000)
...
def plot_data(results, mode):
    ...
    plt.xlabel("Num Jobs")
    plt.ylabel("Mean Read Latency (μs)")
    plt.title(f"Read Latency vs Num Jobs")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"read-latency_{mode}.png")
    plt.show()
